...
export function Shell({ user, demo, children }: { user: Actor; demo: boolean; children: ReactNode }) {
  const path = usePathname(); const router = useRouter(); const [open, setOpen] = useState(false); const [notificationsOpen, setNotificationsOpen] = useState(false); const [error, setError] = useState(""); const [busy, setBusy] = useState(false);
  const notifications = useResource<{ id: string; title: string; body: string; read: boolean }[]>("notifications");
  async function changeRole(role: string) { setBusy(true); setError(""); try { await api("auth/demo", { role }); router.push(roleHome[role]); router.refresh(); } catch (error) { setError((error as Error).message); } finally { setBusy(false); } }
